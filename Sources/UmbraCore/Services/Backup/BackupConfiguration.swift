...
@Observable
public final class BackupConfiguration: Codable, Sendable {
    // MARK: - Types
    ...
    /// Compression type
    public enum CompressionType: String, Codable, Sendable {
        /// No compression
        case none
        /// ZIP compression
        case zip
    ...
    public func validate() throws -> Bool {
        try validateName()
        try validateSourcePaths()
        try validateStorageLocations()
        try validateRetentionPolicy()
        return true
    }
    ...
    public func getStorageLocation(for url: URL) -> StorageLocation? {
    ...
    public func isPathExcluded(_ path: String) -> Bool {
        exclusionPatterns.contains { pattern in
            (try? Regex(pattern).wholeMatch(in: path)) != nil
        }
    }
    ...
    private func validateSourcePaths() throws {
        guard !sourcePaths.isEmpty else {
            throw BackupError.invalidConfiguration("Source paths cannot be empty")
        }
    }

    private func validateStorageLocations() throws {
        guard !storageLocations.isEmpty else {
            throw BackupError.invalidConfiguration("Storage locations cannot be empty")
        }
    }

    private func validateRetentionPolicy() throws {
        guard retentionPolicy.maxBackups > 0,
            ...
            throw BackupError.invalidConfiguration("Invalid retention policy values")
        }
    }
}
